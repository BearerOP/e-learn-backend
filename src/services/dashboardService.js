...
const getInstructorDashboard = async (req, res) => {
    ...
    try {
        // Get courses count
        const coursesCount = await Course.countDocuments({ createdBy: instructorId });
        ...
        return {
             success: true, status: 200, message: "Course added to cart",
            coursesCount,
            totalStudents,
            totalEarnings,
        };
...
const getStudentDashboard = async (req, res) => {
    ...
    try {
        ...
        const user = await User.findById(studentId).select('purchasedCourses').populate('purchasedCourses', 'price');
        ...
        const totalSpent = user.purchasedCourses.reduce((sum, course) => sum + course.price, 0);
